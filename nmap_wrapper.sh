...
rm -f $targetfile
# redis-cli GET last_services | jq -r '[.[] | {ip:.value.host.ip, port:.value.port}] | group_by(.ip)[] | {ip:.[0].ip, ports: [.[].port]} | .ip + " -p T:" + (.ports | join(","))' | sort > $targetfile
...
do
    sudo nmap -sV -Pn -oX out/nmap_report_$(echo $target | cut -d " " -f1).xml $target
done < "$targetfile"
